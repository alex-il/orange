...
public class EnumerateServices {
	...
	/**
	 * The credentials for an administrative user with permission to read one or
	 * more services.
	 */
	...
	public static void main(final String[] args) {
		try {
			...
			System.err.println("---------" + serviceAccessor.get("policies","ttt_ttt").getServiceDetail().getName());
			...
			System.out.println("-----------InvalidOptionException----------");
			e.printStackTrace();
		} catch (AccessorException e) {
			// Handle "business logic" errors
			System.out.println("-----------AccessorException----------");
			e.printStackTrace();
		} catch (ManagementRuntimeException e) {
			// Handle any runtime errors, not meaninfully handled by lower level code
			System.out.println("-----------ManagementRuntimeException----------");
			e.printStackTrace();
		}
	}
